; -----------------------------------------------------------------------------
; .del - standalone NextZXOS dot command
; -----------------------------------------------------------------------------
; Deletes files/directories recursively using the esxDOS-compatible API.
; This command shares the path parser, LFN traversal, BREAK handling, status
; screen, turbo switching and ROM error handler style with .copy.
; -----------------------------------------------------------------------------
...
MAIN
        ...
        ; Source is mandatory. The parser accepts either `.del source` or
        ; `.del -s source`. Quoted paths are accepted for names with spaces.
        ld a,(srcPath)
        or a
        jp z,show_usage_error
        ld a,(dstPath)
        or a
        jp nz,show_usage_error
        ld a,(moveFlag)
        or a
        jp nz,show_usage_error
        ld a,(yesFlag)
...
delete_one_file
        ; Delete a single non-directory path. Directory deletion uses delete_dir.
        ld hl,1
        ld (totalFiles),hl
        ld ix,(ctxPtr)
        ld hl,0
...
setup_delete_context
        ; .del uses only the source side of the shared context. LFN_NAME also
        ; points to srcName so existing status formatting prints the deleted root.
        ld hl,msgDbgSetupStart
        call debug_print_msg
        ld a,SYSCOPY_ABI
        ld (dotCtx+SYSCOPYCTX_ABI),a
...
build_root_src_path
        ; Build SRC_STACK[0] = srcParent/srcName for recursive deletion.
        ld a,STAGE_ROOT_PATH
        ld (failStage),a
        ld ix,(ctxPtr)
        ld l,(ix+SYSCOPYCTX_SRC_PATH)
...
; HL=path being deleted. Shows a stable "current item" row without clearing
; the whole screen, so deleting many small files does not flicker.
print_delete_path
        push af
        push bc
        push de
        push hl
        ld de,msgDeletingNow
        ld hl,0*256+8
        ld a,16
        call call_print
        pop de
...
msgUsageError defb "del: missing or bad arguments",13,0
msgTitle    defb ".del - by Shrek/MB Maniax 2026",0
msgSource   defb "Target:",0
...
msgHelp
        defb ".del - by Shrek/MB Maniax 2026",13
        defb 13
        defb "DEL 0.1 - recursive delete",13
        defb "Usage:",13
        defb "  .del target",13
        defb "  .del -s target",13
        defb "Options:",13
        defb "  -s  source/target path",13
        defb "  -h  show this help",13
        defb 13
        defb "Directories are deleted recursively.",13
        defb "BREAK cancels between files.",13
        defb 13
        defb "Examples:",13
        defb "  .del tmp",13
        defb "  .del d:/backup",13
        defb "  .del ",34,"old folder",34,13
        defb "  .del -s ",34,"d:/old backup",34,13,0

copyPhaseTxt   defb "Copying: ",0
deletePhaseTxt defb "Deleting: ",0
msgDeletingNow defb "Deleting: ",0
statusLine     defs 56
...
plugin_end
        ...
        SAVEBIN "extra/del", MAIN, plugin_end - MAIN
        END MAIN
